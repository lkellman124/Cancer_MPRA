...
Using merged loops files:
/oak/stanford/groups/khavari/users/mguo123/pan_omics/data/interim/merged/loops/Airway.loops.csv

scp lkellman@khavari.scg.stanford.edu:/oak/stanford/groups/khavari/users/mguo123/pan_omics/data/interim/merged/loops/Airway.loops.csv .

scp -r lkellman@khavari.scg.stanford.edu:/oak/stanford/groups/khavari/users/mguo123/pan_omics/data/interim/merged/loops .

```{r}
files = list.files(path = "~/Documents/MPRA/Cancer_MPRA_Scaleup/eGeneFiles/HiChIP_112820/mergedloops/loops/",
           full.names = T)
cell_types = str_remove(files, "/Users/laurakellman/Documents/MPRA/Cancer_MPRA_Scaleup/eGeneFiles/HiChIP_112820/mergedloops/loops//")
...
gene_table <- read_tsv("~/Documents/MPRA/Cancer_MPRA_Scaleup/eGeneFiles/HiChIP_112820/ucsc_resfseq_refseqall",
...
```{r}
res_merge <- read_tsv("~/Documents/MPRA/Cancer_MPRA_Scaleup/Analysis11.12.20/Analysis11.27.20/res_merge_plasmidrep_reanalyzed_112820.tsv")
snp_positions <- dplyr::select(res_merge, Causal_SNP, Chr_37, Start_37) %>% distinct()
snp_positions$Start_37 <- as.numeric(snp_positions$Start_37)
```
...
for (i in 1:dim(snp_positions)[1]){
  x = snp_positions[i,]
  snp_checked = check_snp(x["Causal_SNP"], x["Chr_37"], x["Start_37"], anchors)
...
```{r}
write_tsv(loops_all, "~/Documents/MPRA/Cancer_MPRA_Scaleup/eGeneFiles/HiChIP_112820/loops_frommerged_112920.tsv")

write_tsv(loops_all, "~/Documents/MPRA/Cancer_MPRA_Scaleup/eGeneFiles/HiChIP_112820/loops_rerunforloop_frommerged_113020.tsv")

write_tsv(loops_all, "~/Documents/MPRA/Cancer_MPRA_Scaleup/eGeneFiles/HiChIP_112820/loops_rerunforloop_frommerged_113020_check.tsv")
loops_all_old <- read_tsv("~/Documents/MPRA/Cancer_MPRA_Scaleup/eGeneFiles/HiChIP_112820/loops_rerunforloop_frommerged_113020.tsv")

loops_all_old <- read_tsv("~/Documents/MPRA/Cancer_MPRA_Scaleup/eGeneFiles/HiChIP_112820/loops_frommerged_112920.tsv")
```


Separate out into loops and same
```{r}
...
```{r}
write_tsv(loops_snpcell, "~/Documents/MPRA/Cancer_MPRA_Scaleup/eGeneFiles/HiChIP_112820/mergedloops/hichiploops_looponly_snpcellgrouped_113020.tsv")
...
```{r}
write_tsv(same_bin_snpcell, "~/Documents/MPRA/Cancer_MPRA_Scaleup/eGeneFiles/HiChIP_112820/mergedloops/hichip_samebin_snpcellgrouped_112920.tsv")
...
snp_disease <- res_merge %>% dplyr::filter(!(Disease_expanded %in% c("bladder", "cervical") &
                                               Disease_original %in% c("bladder", "cervical"))) %>%
  dplyr::select(Causal_SNP, Disease_expanded, Disease_original) %>%
...
snp_disease <- snp_disease %>% mutate(Disease_original = str_split(Disease_original, ",")) %>%
  unnest(cols = Disease_original) %>% 
  mutate(Disease_original = case_when(
        grepl("skin", Disease_original) ~ "kc",
    grepl("lung", Disease_original) ~ "airway",
    grepl("brain", Disease_original) ~ "ast",
    grepl("colon", Disease_original) ~ "colon",
    grepl("endo", Disease_original) ~ "endo",
    grepl("eso", Disease_original) ~ "eso",
    grepl("lymphoma", Disease_original) ~ "gm",
    grepl("brca", Disease_original) ~ "hmec",
    grepl("mel", Disease_original) ~ "mc",
    grepl("ovca", Disease_original) ~ "ov",
    grepl("paad", Disease_original) ~ "panc",
    grepl("prad", Disease_original) ~ "pros",
    grepl("kidney", Disease_original) ~ "renal",
    grepl("thy", Disease_original) ~ "thy"))

snp_disease <- snp_disease %>% group_by(Causal_SNP) %>%
  summarise(disease = paste(sort(unique(c(Disease_original, Disease_expanded))[!is.na(unique(c(Disease_original, Disease_expanded)))]), collapse=","))
```
...
res_merge_hichip <- left_join(res_merge, hichip_correct_celltype,
                              by=c("Causal_SNP"="snp"))
res_merge_hichip <- dplyr::select(res_merge_hichip, Causal_SNP,Disease_original,  
                                  Disease_expanded, HiChIP_egene,
                                  HiChIP_tissue, HiChIP_loop, HiChIP_tss_atac,
                                  hichip_egenes, hichip_ep_loops_cell_type,
                                  hichip_ep_loop, hichip_correct_cell, hichip_correct_cell_types)
...
Find nearest genes
- Add genes that are nearby
- snp within 10kb of the gene
```{r}
...
for (i in 1:dim(snp_positions)[1]){
  x = snp_positions[i,]
  print(i)
  nearest_genes_snp = find_nearest(x[["Causal_SNP"]], x[["Chr_37"]], x[["Start_37"]], gene_loc)
...
nearest_genes <- as.data.frame(nearest_genes)
...
```
Add nearby genes and hichip to res merge

```{r}
colnames(hichip_correct_celltype) <- c("Causal_SNP", "hichip_egene", "hichip_cell_type")
res_merge_hichip <- left_join(dplyr::select(res_merge, -HiChIP_egene, -HiChIP_tissue,
                                            -HiChIP_loop, -HiChIP_tss_atac, -hichip_egenes,
                                            -hichip_ep_loops_cell_type, -hichip_ep_loop),
...
```{r}
write_tsv(res_merge_gene, "~/Documents/MPRA/Cancer_MPRA_Scaleup/eGeneFiles/HiChIP_112820/res_merge_hichip_nearbygenes_update_113020.tsv")
```
