...
#   alpha <- read_tsv("~/Documents/MPRA/Cancer_MPRA_Scaleup/QuantitativeMPRAnalyze/alpha_results_1.28.21.tsv")
  boxplot(alpha)
...
motifs <- alpha[grepl("motif", rownames(alpha)),]
motifs <- motifs[!(grepl("JM", rownames(motifs))),]
# motifs_filt <- motifs[rownames(motifs) %in% keep,]
motifs_sds <- apply(motifs, 1, sd)
quantile(motifs_sds, seq(0,1, 0.1), na.rm=T)
motifs_filt <- motifs[motifs_sds > quantile(motifs_sds, na.rm=T)[4] & !is.na(motifs_sds),]
rownames(motifs_filt) <- str_remove(str_remove(rownames(motifs_filt), "^motif_"), "_motif")

pheatmap(log2(motifs_filt+1), fontsize_row = 3, main = "MPRA transcription levels of motifs in library")
