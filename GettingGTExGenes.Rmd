...
lib <- read_tsv("~/Documents/MPRA/Cancer_MPRA_Scaleup/AnalysisFiles12.5.19/FormattingBackground/GTEx_IDs_eQTLs/lib_studies_idmerge_diseasesexpanded_coords3738_gtexIDuncollapsed_1.9.20.tsv")
```
#### Find all the significant SNP-egene pairs in our tissues from gtex v8
```{r}
file_list <- list.files(path="~/Documents/MPRA/Cancer_MPRA/Lx_Analysis/GTEx_Analysis_v8_eQTL/",
...
write_tsv(egene_pairs, "~/Documents/MPRA/Cancer_MPRA_Scaleup/eGeneFiles/GTEx/significant_egene_pairs_for_snps_in_gtex_tissue_filtered_1.13.20.txt")  

egene_pairs <- read_tsv("~/Documents/MPRA/Cancer_MPRA/Lx_Analysis/significant_egene_pairs_for_snps_in_gtex.txt")
```
...
# human
human_file='~/Documents/MPRA/Cancer_MPRA/Lx_Analysis/geneID_mapping_human.tsv'
...
egene_med <- read_tsv("~/Documents/MPRA/Cancer_MPRA_Scaleup/eGeneFiles/GTEx/Uncollapsed_tissuefilt_egenesnp_pairs.txt")
...
write_tsv(lib_gtex_merge, "~/Documents/MPRA/Cancer_MPRA_Scaleup/AnalysisFiles12.5.19/FormattingBackground/lib_studies_idmerge_diseasesexpanded_coords3738_GTExeGenes_tissuecolumn_1.13.20.tsv")
...
gtex_tissue_filtered <- dplyr::filter(gtex_tissue_filtered, correct_tissue)
gtex_to_join <- dplyr::select(gtex_tissue_filtered, variant_id, hgnc) %>% distinct()
gtex_to_join <- gtex_to_join %>% group_by(variant_id) %>% 
  summarise(gtex_egenes_correct_tissue = paste(sort(unique(hgnc)), collapse=","))


# collapse the egene sets
egenes <- egene_med %>% group_by(variant_id) %>%
  summarise(gtex_egenes = paste(unique(hgnc), collapse=","),
            gtex_egenes_ens = paste(unique(gene_id), collapse=","),
            gtex_tissues = paste(unique(tissue), collapse=","),
            gtex_direction = paste(unique(slope), collapse=","))

duped <- lib_gtex_merge$Causal_SNP[duplicated(lib_gtex_merge$Causal_SNP)]
dedup <- lib_gtex_merge %>% mutate(varid_allele = str_remove(str_remove(variant_id, "_b38"), "^.*_"),
                                   same_allele = varid_allele == Alt)

dedup_lost <- dplyr::filter(dedup, (Causal_SNP %in% duped & !(varid_allele == Alt)))
length(unique(lib_gtex_merge$variant_id))
dedup <- dedup %>% group_by(Causal_SNP, Alt) %>%
  summarise_all(function(x)
    ifelse(length(x) > 1, paste(sort(unique(x)), collapse=","), x))



lib_gtex_merge <- left_join(lib, egenes, by=c("variant_id"="variant_id"))
lib_gtex_merge <- left_join(lib_gtex_merge, gtex_to_join)



length(unique(lib_gtex_merge$locus_name))

    variant_id_test = ifelse(length(variant_id) > 1,
                                  paste(sort(unique(variant_id)), collapse=","),
                                  variant_id)
                            
  
  dplyr::filter(dedup, !(Causal_SNP %in% duped & !(varid_allele == Alt)))
sum(dedup_lost$locus_name %in% dedup$locus_name)
length(unique(dedup_lost$locus_name))
