---
title: "Untitled"
output: html_document
---

```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = TRUE)
```

# Want to re-run and verify linkage and risk allele assignments.
## SAVE LINKAGE FOR A SEPARATE FILE
#### For linkage:
* Make sure every SNP is annotated with all lead SNPs with which it's in LD > 0.8
* Take a look at the alignments between causal SNP alleles and lead snp alleles
        * sometimes the allele frequency makes it seem like it's not so good
* I think I mostly took care of this with the disease expansion
        
#### For risk alleles:
* For lead SNPs, determine whether risk allele is ref, alt or unknown based on GWAS entries
* For each causal allele, determine which allele is risk 


### Files from last time for risk allele:
HaploReg_FindingSNPs
make_disease_allele
lib_risk_key


### Major files from last attempt are:
"~/Documents/MPRA/Cancer_MPRA/HaploReg/cmpra_haploreg_data_withquery.tsv"
"~/Documents/MPRA/Cancer_MPRA/HaploReg/haploreg_with_gwas_risk_for_lead_snps.tsv"

### PLAN:
Get the haploreg data for lead and causal snps like in HaploReg_FindingSNPs






```{r}
lib <- read_tsv("~/Documents/MPRA/Cancer_MPRA_Scaleup/AnalysisFiles12.5.19/FormattingBackground/lib_table_merged_rsids_12.10.19.tsv")
lib_bg <- read.table("~/Documents/MPRA/Cancer_MPRA_Scaleup/LibraryFiles/YZ_SNP_Files/cancer/all-causal-SNP-16cancers.txt", 
                       stringsAsFactors = F, sep="\t", header=T )
gwas_cat <- read_tsv("~/Documents/MPRA/Cancer_MPRA/HaploReg/gwas_catalog_v1.0-associations_e96_r2019-10-14.tsv")
```

If strongest SNP risk allele is ? then unknown
```{r}
sum(grepl("\\?", gwas_cat$`STRONGEST SNP-RISK ALLELE`))
causal_snps <- unique(unlist(str_split(lib$Causal_SNP, ",")))
lead_snps <- unique(unlist(str_split(lib$leadSNP, ",")))

lib_snps <- unique(c(causal_snps, lead_snps))
gwas_lib_cat <- dplyr::filter(gwas_cat, SNPS %in% lib_snps)
                              
# checking to see if everything worked out                             
sum(lead_snps %in% gwas_lib_cat$SNPS)
sum(causal_snps %in% gwas_lib_cat$SNPS)

# 22 lead snps left out. Largely from a prostate interaction study that formatted the snps as snp x snp
# a lot of causal snps have no lead snp
check <- lead_snps[!(lead_snps %in% gwas_lib_cat$SNPS)]
checklib <- lib[lib$leadSNP %in% check,]
checklib <- lib_bg[lib_bg$LeadSNP %in% check,]
checklib <- checklib[!is.na(checklib$LeadSNP),]
gwas_filt <- gwas_cat
checkgwas <- gwas_cat[unique(unlist(sapply(unique(checklib$LeadSNP), function(x) grep(x, gwas_cat$SNPS)))),]
sapply(unique(checklib$LeadSNP), function(x) sum(grepl(x, checkgwas$SNPS)))

# all of the missing lead snps are also missing the useful allele information
# will just label them unknown

# what other disease are our snps linked to?
check_disease <- dplyr::filter(gwas_lib_cat, !(grepl("cancer|carcinoma", `DISEASE/TRAIT`) | grepl("cancer", STUDY)))
# sort by disease so we don't get the allele linked to height or something
gwas_lib_filt <- gwas_lib_cat %>% dplyr::filter(grepl("ancer|arcinoma|ymphoma|lioma|eukemia|elanoma|lioblastoma|adenoma", `DISEASE/TRAIT`) | grepl("ancer", STUDY))
```
Getting the allele out of strongest risk allele
```{r}
gwas_lib_filt <- gwas_lib_filt %>% mutate(gwas_allele = str_remove(`STRONGEST SNP-RISK ALLELE`,
                                                                 "^.*-"))

```

### Relating allele and risk
OR or BETA can be an OR or a beta. An OR should be greater than 1, and the alleles should have been flipped to correspond. A beta can be greater or less than 1 (centered on 0), and the CI column should say increasing or decreasing
Decreasing will not be negative

```{r}
gwas_lib_filt <- gwas_lib_filt %>% 
  mutate(risk_allele = case_when(
    gwas_allele == "?" ~ "unknown",
    is.na(`OR or BETA`) ~ "unknown",
    grepl("increas", `95% CI (TEXT)`) ~ gwas_allele,
    grepl("decreas", `95% CI (TEXT)`) ~ paste("flip", gwas_allele, sep="_"),
    `OR or BETA` <  1 ~ "unknown",
    `OR or BETA` > 1 ~ gwas_allele,
    T ~ "unknown"            
  ))

duplicates <- gwas_lib_filt[gwas_lib_filt$SNPS %in% gwas_lib_filt$SNPS[duplicated(gwas_lib_filt$SNPS)],]
gwas_lib_coll <- gwas_lib_filt %>% group_by(SNPS) %>% 
  summarise(gwas_risk_allele = paste(risk_allele, collapse=","))
lead_gwas <- dplyr::select(gwas_lib_filt, SNPS, risk_allele)
           
```


```{r}
write_tsv(gwas_lib_coll, "~/Documents/MPRA/Cancer_MPRA_Scaleup/AnalysisFiles12.5.19/FormattingBackground/FindingRiskAllele/gwas_risk_allele_key_leadsnps_veryunreliable_1.14.20.tsv")

```


Now I have the probable-ish risk alleles for the lead snps. Want to line them up with whether they're the reference or alternate allele for that snp (haploreg) and then add the ref/alt info into the lib table

```{r}
haplo <- read_tsv("~/Documents/MPRA/Cancer_MPRA_Scaleup/AnalysisFiles12.5.19/Linkage_and_risk_allele/HaploReg/cmpra_haploreg_data_withquery.tsv")
rc_check <- function(x) {
  return(case_when(
    x == "A" ~ "T",
    x == "T" ~ "A",
    x == "C" ~ "G",
    x == "G" ~ "C"
  ))
}

haplo_gwas <- right_join(haplo, lead_gwas, by=c("rsID"="SNPS"))
haplo_gwas <- haplo_gwas %>% mutate(risk_allele_ref_alt = case_when(
  risk_allele == "unknown" ~ "unknown",
  risk_allele == ref ~ "ref",
  risk_allele == alt ~ "alt",
  !grepl(",", alt) & rc_check(risk_allele) == ref ~ "ref",
  !grepl(",", alt) & rc_check(risk_allele) == alt ~ "alt",
  T ~ "unknown"))

write_tsv(haplo_gwas, "lead_snp_risk_allele_haplo_gwas_comparisons_1.14.20.tsv")
haplo_gwas <- read_tsv("lead_snp_risk_allele_haplo_gwas_comparisons_1.14.20.tsv")
```

Merge into library table
```{r}
lib <- read_tsv("~/Documents/MPRA/Cancer_MPRA_Scaleup/AnalysisFiles12.5.19/FormattingBackground/lib_studies_idmerge_diseasesexpanded_coords_GTEx_hichip_1.13.20_updated.tsv")

haplo_gwas_coll <- haplo_gwas %>% dplyr::select(rsID, risk_allele_ref_alt) %>% distinct() %>%
  group_by(rsID) %>% summarise(gwas_risk_allele = paste("", risk_allele_ref_alt, sep="", collapse=","))
lib_merge <- left_join(lib, haplo_gwas_coll, by=c("Lead_SNP"="rsID"))
write_tsv(lib_merge, "~/Documents/MPRA/Cancer_MPRA_Scaleup/AnalysisFiles12.5.19/FormattingBackground/lib_studies_idmerge_diseasesexpanded_coords_gtex_hichip_gwasrisk_1.14.20.tsv")
```


