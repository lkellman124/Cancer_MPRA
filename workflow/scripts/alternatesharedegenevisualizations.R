...
## -----------------------------------------------------------------------------------------------------------------
shared <- read_tsv(here("data/enrichment.Process_sharedhconvert_vsuniverse_GOBP.txt"))
...
termplot
ggsave(here("output/alternate_pathway_vis.pdf"), 
       device="pdf", plot = termplot, width = 5.5, height = 4, unit = "in", dpi="print")


## -----------------------------------------------------------------------------------------------------------------
res_merge <- read_tsv(here("output/res_merge_withhitannot_031021.tsv"))
...
pheatmap(egene_mat_t, color=myColor, treeheight_row=0, treeheight_col=0,
         ...
         filename= c(here("output/alternate_common_gene_vis.pdf")), width =4, height = 4)
